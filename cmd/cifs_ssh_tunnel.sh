...
autossh_pids=$(ps aux | grep "autossh" | grep "$local_port:127.0.0.1:" |  grep -v grep | awk '{print $2}')
IFS=$'\n'
...
while true; do
  ssh_tunnel_pid=$(fuser -n tcp $local_port 2>/dev/null| awk -F":" '{print $1}')
  if [ -n "$ssh_tunnel_pid" ]; then
    do_log "ssh tunnel is listening on port $local_port after $elapsed_time seconds"
    exit 0
  else
    current_time=$(date +%s)
